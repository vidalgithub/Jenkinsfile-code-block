...
## Deploy Terraform code using Jenkins
```
pipeline {
    parameters {
...
```
## CHECKOUT A REPOSITORY INTO the ${env.WORKSPACE}
```
        stage('Checkout') {
            steps {
                script {
...
```
## Clone a specific branch into a specific local directory in the ${env.WORKSPACE}
```
        stage('Checkout') {
            steps {
                script {
...
```
## Example Jenkinsfile to provision infrastructure in AWS using Docker agent, and dynamic AWS IAM credentials generated by Vault
## with Approval stage
```
pipeline {
    agent {
...
```
## Ezxample Jenkinsfile to provision EKS INFRASTRUCTURE in AWS using Docker agent, and  static AWS IAM credentials managed by Vault
## with Approval stage
```
pipeline {
    agent {
...
```
## Example provisionInfrastructure(), and destroyInfrastructure() functions to provision infrastructure WITH TERRAFORM
## with Approval stage
```
def provisionInfrastructure() {
    try {
